...
_JD_PREFERRED_KEYWORDS = re.compile(
    r"\b(?:nice\s+to\s+have|preferred|bonus|plus|desired|optional)\b",
    re.IGNORECASE,
)
...
def _extract_jd_required_skills(text: str, taxonomy: list[str]) -> list[str]:
    ...
    if not search_text:
        return []
    pref_parts = _JD_PREFERRED_SECTION.split(search_text, maxsplit=1)
...
def structure_jd(text: str) -> dict[str, Any]:
    ...
    has_required_section = bool(_JD_REQUIRED_SECTION.search(text))
    has_preferred_section = bool(_JD_PREFERRED_SECTION.search(text))

    if has_required_section or has_preferred_section:
        required = _extract_jd_required_skills(text, taxonomy) if has_required_section else []
        preferred = _extract_jd_preferred_skills(text, taxonomy) if has_preferred_section else []
        if not required:
            required = [s for s in all_skills if s not in preferred]
        preferred = [s for s in preferred if s not in required]
    else:
        required, preferred = _bucket_jd_skills_heuristic(text, all_skills)
